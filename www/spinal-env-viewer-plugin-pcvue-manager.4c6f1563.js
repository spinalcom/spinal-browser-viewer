...
var _spinalModelBmsnetwork = require("spinal-model-bmsnetwork");
async function linkDevicesToFloorId(geographicContextId1, geographicStartId1, pcVueDevices1, func1, pcvueContextId1, pcvueStartId1) {
    const floors1 = await getFloors(geographicContextId1, geographicStartId1);
    const devices1 = pcVueDevices1 && Array.isArray(pcVueDevices1) && pcVueDevices1.length > 0 ? pcVueDevices1 : await getDevices(pcvueContextId1, pcvueStartId1);
    const map1 = getMap(floors1, devices1, func1);
    const promises1 = Array.from(map1.keys()).map((key1)=>{
        const ids1 = map1.get(key1);
        return createNodeLinks(key1, ids1);
    });
    return Promise.all(promises1);
}
async function createNodeLinks(parentId1, childrenIds1) {
    try {
        const promises1 = childrenIds1.map((childId1)=>(0, _spinalEnvViewerGraphService.SpinalGraphService).addChild(parentId1, childId1, (0, _spinalModelBmsnetwork.SpinalBmsDevice).relationName, (0, _spinalEnvViewerGraphService.SPINAL_RELATION_PTR_LST_TYPE)));
        return Promise.all(promises1);
    } catch (error1) {
        return false;
    }
}
function getFloors(geographicContextId1, geographicStartId1) {
    return findInContext(geographicContextId1, geographicStartId1, (0, _constants.FLOOR_TYPE));
}
function getDevices(pcvueContextId1, pcvueStartId1) {
    return findInContext(pcvueContextId1, pcvueStartId1, (0, _spinalModelBmsnetwork.SpinalBmsDevice).nodeTypeName);
}
...
function getMap(floors1, devices1, func1) {
    const floorsCopy1 = Object.assign([], floors1);
    const devicesCopy1 = Object.assign([], devices1);
    const map1 = new Map();
    while(floorsCopy1.length > 0 && devicesCopy1.length > 0){
        const floor1 = floorsCopy1.shift();
        const indexes1 = getIndexes(floor1.name, devicesCopy1, func1);
        const res1 = [];
        for (let idx1 of indexes1){
            const item1 = devicesCopy1[idx1];
            res1.push(item1.id);
        }
        map1.set(floor1.id, res1);
    }
    return map1;
}
function getIndexes(floorName, devices, callback) {
    return devices.map((el, idx)=>eval(`(${callback})(floorName, el.name)`) ? idx : undefined).filter((el1)=>typeof el1 !== "undefined");
}
