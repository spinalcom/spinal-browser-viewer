      ...
      validator: (val) =>
        val.every((shortcut) => {
          ...
          return (
            shortcut.key &&
            shortcut.label &&
            (isValueInteger || isFunction
              ? true
              : SHORTCUT_TYPES.includes(shortcut.value))
          );
        }),
    },
  ...
  methods: {
    init() {
      ...
      if (this.value) {
        const selectedShortcut = this.customShortcuts.find(
          (shortcut) => shortcut.key === this.value
        );
    ...
    noticeDeprecation() {
      const useDeprecatedAPI = this.customShortcuts.find(
        (shortcut) =>
          typeof shortcut.isSelected !== 'undefined' ||
          typeof shortcut.key === 'undefined'
      );
      if (useDeprecatedAPI)
        console.warn(
    ...
    getShortcutByKey(shortcutKey) {
      const shortcut = this.customShortcuts.find(
        (sc) => sc.key === shortcutKey
      );
      ...
      if (typeof value === 'function') {
        ...
        if (!moment.isMoment(start) || !moment.isMoment(end))
          throw new Error(
            'The "start" or "end" values are not moment objects.'
          );
    ...
    select(shortcut) {
      ...
      const { start, end, value } = this.getShortcutByKey(
